#!/usr/bin/env python3
"""Orchestrate semantic block reconstruction from MinerU outputs."""
...
def classify_block(
    ...
    if is_probable_body_section(text, domain_profile) and item_type in {"text", "aside_text", "image"}:
    ...
    if item_type in {"text", "aside_text"}:
        ...
        if item.get("text_level") or is_probable_major_heading(text, domain_profile):
...
def build_for_output_dir(
    ...
    llm_confidence_threshold: float = LLM_REVIEW_CONFIDENCE_THRESHOLD,
    candidate_min_score: float = HEADING_CANDIDATE_MIN_SCORE,
    ...
    toc_nodes = parse_toc_tree(raw_wrapped_items, domain_profile)
    ...
    item_regions = classify_item_regions(wrapped_items, domain_profile)
    ...
    rule_decisions = build_rule_decisions(candidates, toc_levels, toc_paths, domain_profile)
    ...
    with structured_path.open("w", encoding="utf-8") as jsonl:
        for wrapped in wrapped_items:
            ...
            if block_type == "image":
                ...
                image_table_candidate = any(keyword in probe for keyword in keywords)
    ...
    diagnostics = heading_diagnostics(structured_blocks, decisions, domain_profile)
