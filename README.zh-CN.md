...
- `section_tree.json`：正文章节树旁路产物，优先使用正文标题恢复父子关系；当正文标题不足时，使用 TOC backbone 兜底，后续可作为反写 Markdown 层级和 RAG 分块的权威结构来源。
...
- 章节树：生成 `section_tree.json` 作为长期稳定的正文父子关系层；当前阶段不改变 `.semantic.md` 标题输出，只做旁路建树和质量门控。
